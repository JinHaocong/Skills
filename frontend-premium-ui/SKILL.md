...
- 主智能体负责最终视觉方向、信息层级、组件系统、工程落地和验收口径。
- 大页面可以先拆成视觉系统、页面骨架、核心组件、数据状态、响应式与视觉 QA 六个工作面。
- 用户偏好是自动多智能体：当页面或视觉任务达到中等规模且当前运行/工具策略允许时，默认创建一个或多个子智能体；只要存在参考审计、视觉系统、页面骨架、独立组件、响应式检查或截图 QA 中的两个以上工作面，就视为适合分发。
- 子智能体不得自行改变主视觉方向、品牌色、布局密度或组件语义；这些决策由主智能体统一。
- 本地项目改动后，主智能体应尽量启动或使用已有 dev server，并用浏览器检查主要视口，确认布局、文本、交互状态和视觉资产没有明显问题。
