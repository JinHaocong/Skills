...
- For medium-or-larger ports, split the work into source inventory, target convention audit, API/types adaptation, state migration, page/component migration, and route/permission integration.
- User preference: use automatic subagent delegation for medium-or-larger ports when the current runtime/tool policy permits spawning agents. Treat any port with separate source inventory, target convention audit, API/types, state, UI, route, or permission work as delegation-worthy.
- Assign read-only inventory tasks or disjoint write scopes such as API/types versus UI components.
